...
class _ActivityOrLegacyActionHandler:
    """Dispatch marked one-step Workflows to Activities during migration."""

    def __init__(self, legacy: DeviceExecutionActionHandler, activity: ActivityActionHandler) -> None:
        self.operation = legacy.operation if hasattr(legacy, "operation") else activity.operation
        self._legacy = legacy
        self._activity = activity

    async def execute(self, action: ActionSpec, run: WorkflowRun, emit: Any) -> ActionResult:
        if bool(action.params.get("_framework_activity", False)):
            return await self._activity.execute(action, run, emit)
        return await self._legacy.execute(action, run, emit)

    async def cancel(self, action: ActionSpec, run: WorkflowRun) -> None:
        if bool(action.params.get("_framework_activity", False)):
            await self._activity.cancel(action, run)
            return
        await self._legacy.cancel(action, run)
    ...
    *,
    ...
    for operation in legacy_operations:
        ...
        if activity_executor is not None and operation in {
            ...
            selected = _ActivityOrLegacyActionHandler(
                handler,
    ...
    # immediately.  Generic device Activities are registered here as well;
    # vendor command generation remains inside the compatibility bridge until
    # the corresponding adapter is extracted.
