"""Huawei VRP implementation of the legacy framework Action boundary.

This adapter is kept for compatibility while callers migrate to native
Activities. Vendor command generation, output parsing, and device interaction
belong in the infrastructure adapter package.
"""
...
from device_tui.framework.models import ActionResult, ActionSpec, ActionStatus, WorkflowRun
from device_tui.framework.plugins import AdapterRegistry


class DeviceExecutionPort(Protocol):
    """Minimal execution surface required by the vendor Action adapter."""
...
class _CommandStep:
    """Structural command value accepted by the DeviceExecutionPort."""
...
class DeviceExecutionActionHandler:
    ...
    async def _legacy_step(self, action: ActionSpec, run: WorkflowRun) -> _CommandStep:
        ...
        if operation == "device.probe":
            driver = self._driver(action, run)
            params["commands"] = driver.probe_commands(
            ...
            return _CommandStep(action.id, kind="device", action="precheck", params=params)
        if operation == "device.verify":
            ...
            if "commands" not in params:
                params["commands"] = self._driver(action, run).verification_commands(fact)
            if fact == "startup_package" and "expected_package" not in params:
                params["expected_package"] = params.get("expected")
            return _CommandStep(action.id, kind="device", action=action_name, params=params)
        if operation == "huawei.startup.configure":
            ...
            return _CommandStep(action.id, kind="device", action="activate", params=params)
        if operation == "huawei.storage.cleanup":
            params["commands"] = self._cleanup_commands(action, run)
            return _CommandStep(action.id, kind="device", action="command", params=params)
        ...
        if operation == "file.transfer":
            ...
            return _CommandStep(action.id, kind="device", action="transfer", params=params)
        if operation == "device.reboot":
            params.setdefault("steps", list(self._driver(action, run).reboot_plan_steps()))
            return _CommandStep(action.id, kind="device", action="reboot", params=params)
        if operation == "device.wait_online":
            params.setdefault(
                "readiness_command",
                self._driver(action, run).version_query_command(),
            )
            ...
            return _CommandStep(action.id, kind="device", action="wait_online", params=params)
        raise DeviceWorkflowExecutionError(
    ...
    def _cleanup_commands(self, action: ActionSpec, run: WorkflowRun) -> tuple[str, ...]:
        ...
        for label, storage in (
            ...
            if plan.delete_paths and not bool(params.get("auto_delete_old_packages", False)):
...
__all__ = ["DeviceExecutionActionHandler", "DeviceExecutionPort"]
